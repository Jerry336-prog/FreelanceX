...
const BrowseJobs = () => {
  ...
  return <section id="browse-jobs" className="py-24 bg-white">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <form onSubmit={(event) => { event.preventDefault(); setQuery(search.trim()); }} className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm mb-10 flex gap-3">
      ...
      {loading ? <div className="py-16 flex justify-center"><Loader2 className="w-7 h-7 animate-spin text-blue-600" /></div> : jobs.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-slate-50 p-12 text-center text-slate-500">No open jobs match your search yet.</div> : <div className="space-y-4">{jobs.map((job) => {
        ...
        return <article key={job._id || job.id} className="border border-slate-200 rounded-2xl p-6 hover:shadow-lg transition-shadow"><div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6"><div className="flex-grow"><h3 className="text-xl font-bold text-slate-900 mb-2">{job.title}</h3><div className="flex flex-wrap items-center text-sm text-slate-500 gap-4 mb-4"><span className="flex items-center text-slate-700 font-medium"><MapPin className="w-4 h-4 mr-1 text-slate-400" />{clientName}</span><span className="flex items-center"><Clock className="w-4 h-4 mr-1" />{timeAgo(job.createdAt)}</span><span className="flex items-center"><DollarSign className="w-4 h-4 mr-1" />{job.category || 'Project'}</span></div><div className="flex flex-wrap gap-2">{(job.skills || []).slice(0, 6).map((skill) => <span key={skill} className="px-3 py-1 bg-slate-100 text-slate-600 text-xs font-semibold rounded-full">{skill}</span>)}</div></div><div className="flex flex-row lg:flex-col items-center lg:items-end justify-between lg:w-48 border-t lg:border-t-0 lg:border-l border-slate-100 pt-4 lg:pt-0 lg:pl-6"><p className="text-lg font-bold text-slate-900">{budget}</p><button type="button" onClick={() => setShowAccessModal(true)} className="mt-3 px-5 py-2.5 bg-white border-2 border-slate-200 text-slate-700 text-sm font-semibold rounded-xl hover:border-blue-600 hover:text-blue-600">View Job</button></div></div></article>;
      })}</div>}
    </div>
  </section>;
};
