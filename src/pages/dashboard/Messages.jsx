...
import {
  ...
  CheckCheck, Check, ArrowLeft, Plus, X, UserPlus, Briefcase, Trash2, CheckCircle, ShieldCheck, Paperclip,
  FileText, Download, Image as ImageIcon, Eye, ExternalLink
} from 'lucide-react';
import { messageService, contractService, proposalService } from '../../services/api';
...
const avatarInitials = (p) =>
  `${(p.firstname || p.name || '?')[0]}${(p.lastname || '')[0] || ''}`.toUpperCase();
...
const Avatar = ({ user, size = 10 }) => {
  ...
  const base = `${dim} rounded-full overflow-hidden flex items-center justify-center text-slate-600 font-bold flex-shrink-0 text-sm`;
  ...
  return <div className={base}><span>{avatarInitials(user || {})}</span></div>;
};

// ─── Main Component (Freelancer Messages) ─────────────────────────────────────
const Messages = () => {
  ...
  const handleConfirmPayment = async (msg) => {
    const activeConv = conversations.find((c) => (c._id || c.id) === activeConvId);
    const contractId = activeConv?.contractId?._id || activeConv?.contractId || activeConv?.jobId;
    if (!contractId) {
      setMsgError("Contract ID is required to confirm payment. Please confirm from the Contract Details page.");
      return;
    }
    setConfirmingId(msg._id);
    setMsgError('');
    try {
      await contractService.confirmPayment(contractId);
      setConfirmedMap((prev) => ({ ...prev, [msg._id]: true }));
      loadConversations(true);
    } catch (err) {
      setMsgError(err?.message || "Unable to confirm payment receipt");
    } finally {
  ...
  const loadConversations = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const seen = new Set();
      const uniqueById = rawList.filter((c) => {
        const cid = c._id || c.id;
        if (seen.has(cid)) return false;
        seen.add(cid);
        return true;
      });
      setConversations(dedupeConversationsByParticipant(uniqueById, myId));
    } catch (err) {
      if (!silent) setConvError(err?.message || 'Failed to load conversations');
    } finally {
    ...
    try {
      ...
      setMessages((prev) => {
        ...
        if (
          ...
          prev.every((m, i) => m._id === msgs[i]?._id && m.read === msgs[i]?.read)
        ) return prev;
        return msgs;
      });
    } catch (err) {
  ...
  const startPolling = useCallback((convId) => {
    ...
    msgPollRef.current = setInterval(() => loadMessages(convId, true), 3000);
    // Poll conversation list every 8 seconds to update previews + unread counts
    convPollRef.current = setInterval(() => loadConversations(true), 8000);
  ...
  useEffect(() => {
    ...
    const initConversation = async () => {
      try {
        const conv = await messageService.getOrCreateConversation(recipientId, contractId);
        if (conv?._id || conv?.id) {
          ...
          if (action === 'revision_inquiry') {
            let title = 'our contract';
            if (contractId) {
              try {
                const cData = await contractService.getContractById(contractId);
                if (cData?.title) title = `"${cData.title}"`;
              } catch (e) {
                console.error('Fetch contract title error:', e);
              }
            }
            setMessageText(`Hi! I saw your revision request for ${title}. Could you please share what specific updates or additions you would like me to make so I can adjust the deliverables accordingly?`);
          }
        }
      } catch (err) {
        console.error('Error opening conversation:', err);
        setConvError(err?.response?.data?.message || err?.message || 'Could not open conversation with this user');
      }
    };
    initConversation();
  ...
  useEffect(() => {
    loadConversations();
    // Poll conversation list even when no conversation is open
    convPollRef.current = setInterval(() => loadConversations(true), 10000);
    return () => {
      if (convPollRef.current) clearInterval(convPollRef.current);
    };
  }, [loadConversations]);
  ...
  const selectConversation = (convId) => {
    ...
    setMsgError('');
    setShowMobileThread(true);
    // Update unread count optimistically to 0 in sidebar
    setConversations((prev) =>
      prev.map((c) => ((c._id || c.id) === convId ? { ...c, unreadCount: 0 } : c))
    );
    ...
    try {
      ...
      (Array.isArray(contractsData) ? contractsData : []).forEach((c) => {
        ...
        if (client && (client._id || client.id)) {
          ...
          if (cid !== myId && !clientMap.has(cid)) {
            clientMap.set(cid, {
              userId: cid,
              name: [client.firstname, client.lastname].filter(Boolean).join(' ') || client.companyName || 'Client',
              email: client.email,
              companyName: client.companyName,
              profileImage: client.profileImage,
              context: `Contract: ${c.title || 'Active Contract'}`,
            });
          }
        }
      });

      (Array.isArray(proposalsData) ? proposalsData : []).forEach((p) => {
        const client = p.jobId?.clientId;
        if (client && (client._id || client.id)) {
          const cid = client._id || client.id;
          if (cid !== myId && !clientMap.has(cid)) {
            clientMap.set(cid, {
              ...
              email: client.email,
              profileImage: client.profileImage,
              context: `Proposal: ${p.jobId?.title || 'Applied Project'}`,
            });
          }
        }
      });

      setContacts(Array.from(clientMap.values()));
    } catch (err) {
  ...
  const handleSend = async (e) => {
    ...
    stagedFiles.forEach((item) => {
      if (item.file) {
        ...
        jsonAttachments.push(cleanReceipt);
      }
    });

    if (jsonAttachments.length > 0) {
      formData.append('attachments', JSON.stringify(jsonAttachments));
    }
    ...
    setConversations((prev) =>
      prev.map((c) =>
        (c._id || c.id) === activeConvId
          ? { ...c, lastMessage: currentText, lastMessageAt: new Date().toISOString() }
          : c
      )
    );
    ...
    try {
      ...
      // Refresh conversation list to get accurate lastMessage
      loadConversations(true);
    } catch (err) {
      setMessages((prev) => prev.filter((m) => m._id !== tempId));
      setMessageText(text);
      setMsgError(err?.message || 'Failed to send message. Please try again.');
    } finally {
      setSending(false);
    }
  };
  ...
  // Deleting a chat is private to the current user; it remains available to
  // the other participant and will reappear if either person messages again.
  ...
  const filteredContacts = contacts.filter((c) => {
    ...
    return c.name.toLowerCase().includes(s) || (c.context || '').toLowerCase().includes(s);
  });
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden relative">
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Choose a client from your contracts or proposals</p>
                </div>
              </div>
              <button onClick={() => setShowNewChatModal(false)} className="text-slate-400 hover:text-slate-700 p-1.5 rounded-lg hover:bg-slate-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 border-b border-slate-100">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {chatStartError && (
                <div className="m-3 p-3 bg-red-50 border border-red-200 rounded-xl text-xs text-red-600 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 flex-shrink-0" />
                  <span className="flex-1">{chatStartError}</span>
                  <button onClick={() => setChatStartError('')}><X className="w-3.5 h-3.5" /></button>
                </div>
              )}
              {loadingContacts ? (
                <div className="p-12 flex flex-col items-center justify-center text-slate-400 gap-2">
                  <Loader2 className="w-6 h-6 animate-spin text-blue-600" />
                  <span className="text-xs">Finding your clients...</span>
                </div>
              ) : filteredContacts.length === 0 ? (
                <div className="p-12 text-center">
                  <p className="text-sm font-semibold text-slate-700 mb-1">No clients found</p>
                  <p className="text-xs text-slate-400">Clients from your contracts and proposals will appear here.</p>
                </div>
              ) : (
                <div className="p-2">
                  {filteredContacts.map((contact) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          disabled={isStarting}
                          className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 font-semibold text-xs group-hover:bg-blue-600 group-hover:text-white transition-all flex items-center gap-1 flex-shrink-0"
                        >
      ...
      <div className={`${showMobileThread ? 'hidden md:flex' : 'flex'} w-full md:w-80 flex-shrink-0 border-r border-slate-200 flex-col bg-slate-50`}>
        {/* Header */}
        <div className="p-4 bg-white border-b border-slate-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs flex items-center gap-1.5 transition-colors"
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
        </div>
          ...
          {loadingConvs ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400 p-6 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Start chatting with clients connected to your projects.</p>
              </div>
              <button
                onClick={openNewChatModal}
                className="px-4 py-2 bg-slate-900 hover:bg-slate-700 text-white font-semibold rounded-xl text-xs transition-colors flex items-center gap-1.5"
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-0.5">
                      <p className={`text-sm truncate ${unread > 0 && !isActive ? 'font-bold text-slate-900' : 'font-semibold text-slate-800'}`}>
                        {other.firstname ? `${other.firstname} ${other.lastname || ''}`.trim() : 'Client'}
                      </p>
                      <span className="text-[10px] text-slate-400 flex-shrink-0 ml-1">
                        {formatTime(conv.lastMessageAt || conv.updatedAt)}
                      </span>
                    </div>
                    <p className={`text-xs truncate ${unread > 0 && !isActive ? 'text-slate-700 font-medium' : 'text-slate-500'}`}>
      ...
      <div className={`${showMobileThread ? 'flex' : 'hidden md:flex'} flex-1 flex-col h-full bg-slate-50/30`}>
        {!activeConvId ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-slate-400 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-xs">
                Message clients directly about requirements, milestones, and deliverables.
              </p>
            </div>
            <button
              onClick={openNewChatModal}
              className="mt-1 px-4 py-2 bg-slate-900 hover:bg-slate-700 text-white font-semibold rounded-xl text-xs transition-colors flex items-center gap-1.5"
            >
              <Plus className="w-3.5 h-3.5" />
          ...
          <>
            {/* Thread Header */}
            <div className="h-16 px-4 md:px-5 bg-white border-b border-slate-200 flex items-center justify-between flex-shrink-0">
            ...
            {/* Error Banner */}
            {msgError && (
              <div className="mx-3 mt-2 p-2.5 bg-red-50 border border-red-200 rounded-xl text-xs text-red-600 flex items-center gap-2 flex-shrink-0">
                <AlertCircle className="w-4 h-4 flex-shrink-0" />
                <span className="flex-1">{msgError}</span>
                <button onClick={() => setMsgError('')}><X className="w-3 h-3" /></button>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-2">
              {loadingMessages ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-2">
                  ...
                  <p className="text-xs text-slate-400">Say hello and kick off the conversation!</p>
                </div>
              ) : (
                messages.map((msg, idx) => {
                  const senderId = msg.senderId?._id || msg.senderId?.id || msg.senderId;
                  const isMe = senderId === myId;
                  const showAvatar = !isMe && (idx === 0 || (messages[idx - 1]?.senderId?._id || messages[idx - 1]?.senderId) !== senderId);
                  const isReceiptMsg = (msg.message || '').includes('RECEIPT') || (msg.message || '').includes('Payment Sent') || (msg.message || '').includes('Payment confirmed');
                  const isConfirmed = confirmedMap[msg._id] || (msg.message || '').includes('confirmed receipt');
                    ...
                    >
                      {!isMe && (
                        <div className="w-7 h-7 flex-shrink-0">
                          {showAvatar && <Avatar user={msg.senderId} size={7} />}
                        </div>
                      )}
                      <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] md:max-w-md`}>
                        <div
                          className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                            isReceiptMsg
                              ? 'bg-amber-50 border-2 border-amber-300 text-slate-900 shadow-md'
                              : isMe
                        ...
                        >
                          ...
                          {isReceiptMsg && !isMe && (
                            <div className="mt-3 pt-3 border-t border-amber-200">
                              {isConfirmed ? (
                                <div className="flex items-center gap-1.5 text-xs font-bold text-green-700 bg-green-100 p-2 rounded-xl">
                                  <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0" />
                                  <span>Payment Confirmed by You</span>
                                </div>
                              ) : (
                                <button
                                  type="button"
                                  disabled={confirmingId === msg._id}
                                  onClick={() => handleConfirmPayment(msg)}
                                  className="w-full py-2 px-3 bg-green-600 hover:bg-green-700 text-white font-bold text-xs rounded-xl shadow-sm flex items-center justify-center gap-1.5 transition-colors disabled:opacity-60"
                                >
                                  {confirmingId === msg._id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                        ...
                        <div className="flex items-center gap-1 mt-1 px-0.5 text-[10px] text-slate-400">
