      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {proposals.map((proposal) => {
            ...
            // Derive a clean display name from a Cloudinary URL or plain path
            const getFileName = (url) => {
              try {
                const raw = decodeURIComponent(url.split('/').pop().split('?')[0]);
                // Remove Cloudinary version prefix like v1234567890_
            ...
            return (
              <div
                key={proposalId}
                className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col md:flex-row"
              >
                {/* Freelancer Info Column */}
                <div className="p-6 md:w-1/3 border-b md:border-b-0 md:border-r border-slate-200 bg-slate-50/70 flex flex-col justify-between">
                  <div>
                    {/* If browsing all proposals (no specific job), show which job this is for */}
                    {!id && jobLabel && (
                      <div className="mb-3 px-3 py-1.5 bg-blue-50 border border-blue-100 rounded-lg text-xs font-semibold text-blue-700 flex items-center gap-1.5">
                      ...
                      <div>
                        ...
                        {freelancer.location && (
                          <p className="text-xs text-slate-500 flex items-center mt-1">
                            <MapPin className="w-3 h-3 mr-1 text-slate-400" /> {freelancer.location}
                          </p>
                        )}
                        {freelancer.rating > 0 && (
                          <p className="text-xs text-amber-600 flex items-center gap-1 mt-1 font-medium">
                            <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                            {freelancer.rating?.toFixed(1)} ({freelancer.totalReviews || 0} reviews)
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="pt-4 border-t border-slate-200">
                    <Link
                      to={`/client/freelancers/${freelancer._id || freelancer.id || ''}`}
                      className="w-full block text-center py-2 bg-white border border-slate-200 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 transition-colors text-xs shadow-sm"
                    >
                      View Public Profile
                    </Link>
                  </div>
                ...
                <div className="p-6 md:w-2/3 flex flex-col justify-between space-y-5">
                  <div>
                    <div className="flex flex-wrap items-center gap-4 mb-5">
                      <div className="bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-200">
                        ...
                        <p className="text-lg font-bold text-slate-900">₦{Number(proposal.bidAmount || 0).toLocaleString()}</p>
                      </div>
                      <div className="bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-200">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Delivery Time</p>
                        <p className="text-lg font-bold text-slate-900">{proposal.deliveryTime || 7} Days</p>
                      </div>
                    ...
                    <div>
                      ...
                      <p className="text-slate-700 text-sm leading-relaxed whitespace-pre-wrap bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                          ...
                          {attachments.map((fileUrl, index) => {
                            ...
                            return (
                              <a
                                ...
                                className="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-slate-50/80 hover:bg-blue-50/60 hover:border-blue-200 transition-all group"
                              >
                  ...
                  <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center gap-3">
                    {!isAccepted && !isRejected && (
                      <>
                        <button
                          onClick={() => openAcceptModal(proposalId)}
                          disabled={processingId === proposalId}
                          className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors shadow-sm flex items-center justify-center gap-2 text-sm disabled:opacity-60"
                        >
                          {processingId === proposalId ? (
                        ...
                        <Link
                          to={`/client/messages?recipient=${freelancerIdStr}`}
                          className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 transition-colors flex items-center justify-center gap-2 text-sm shadow-sm"
                        >
                          <MessageSquare className="w-4 h-4" /> Message
                        </Link>

                        <button
                          onClick={() => openRejectModal(proposalId)}
                          disabled={processingId === proposalId}
                          className="px-4 py-2.5 text-slate-500 hover:text-red-600 font-medium transition-colors ml-auto text-sm"
                        >
                          Decline
                        </button>
                      </>
                    ...
                    {isAccepted && (
                      <div className="flex items-center gap-2 text-green-700 text-sm font-bold bg-green-50 px-4 py-2 rounded-xl border border-green-200">
                        <CheckCircle className="w-4 h-4" /> Contract Active
                      </div>
                    )}

                    {isRejected && (
                      <div className="flex items-center gap-2 text-red-600 text-sm font-medium bg-red-50 px-4 py-2 rounded-xl border border-red-100">
                        <X className="w-4 h-4" /> Proposal Declined
                      </div>
                    )}
                  </div>
                </div>
